...
import { checkIdIsNotTaken, createMobileCode } from "./util.js";
const app = express();
...
const newConnection = (id, role) => `${id}${role}`;
    ...
    socket.on("disconnect", () => {
        ...
        if (details) {
            ...
            if (sessions[sessionID]) {
                ...
                setTimeout(() => {
                    const sessionDetail = role === "guest" || role === "host"
                        ? session[role]
                    ...
                    if (sessionDetail?.socketId === socket.id) {
                        ...
                        if (role === "host") {
                            io.emit("UPDATE_SERVERLIST", { sessionID, connectedPlayers: 0 }, true, // if the session is already on the server list client side
                            ...
                            socket.broadcast.emit("UPDATE_SERVERLIST", { sessionID, connectedPlayers: 1 }, true, // if the session is already on the server list client side
