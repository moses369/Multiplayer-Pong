...
const Pong = ({ paddle1Ref, paddle2Ref, resetRound }: Props) => {
  ...
  const { winner, pongStartPos, host, socket, sessionId, local } = useSelector(
    (state: RootState) => ({
  ...
  const animate = () => {
    if (pongRef.current && paddle2Ref.current && paddle1Ref.current) {
      ...
      const changeSpeedDirectionOfPong = (): any => {
        ...
        const pongHeightToAdd = pongRect.height / 5;
        const paddleHeight = {
      ...
      if (host) {
        ...
        !local && socket.emit("SEND_PONG", sessionId, move);
      }
  ...
  useEffect(() => {
    ...
    return () => {
      socket.removeListener("GET_PONG");
    };
  }, []);
